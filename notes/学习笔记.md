...
1. 判断两个值是否为同一个值。可严格区分+0与-0，以及判定NaN
...
3. [Proxy中所有的handler方法](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Proxy)
   ...
   |ownKeys|Object.getOwnPropertyNames、 Object.getOwnPropertySymbols、Object.keys()和Reflect.ownKeys() 方法的捕捉器|
   |apply|函数调用操作|
   |construct|new 操作符|
...
3. Object.getOwnPropertyDescriptors，获取对象中所有属性，包括get、set方法
  ...
  console.log(newPerson)// { firstName: 'tom', lastName: 'kent', fullName: "tomjack", get fullName: ƒ fullName() }
...
## 48. <a name='Generator-1'></a>2-17 Async/Await 语法糖
